...
func createWrappers(w io.Writer, pkgPattern string, functions []fuzz.Func, options wrapperOptions) error {
	if len(functions) == 0 {
...
func createWrapper(w io.Writer, function fuzz.Func, possibleConstructors []fuzz.Func, qualifyAll bool) error {
	...
	if recv == nil {
		...
		if err != nil {
			...
			fmt.Fprintf(os.Stderr, "genfuzzfuncs: failed to determine receiver type: %v: %v", recv, err)
			return nil
		}
...
func constructorReplace(recv *types.Var, possibleConstructors []fuzz.Func) (ctorReplacement, []*types.Var, error) {
	...
	for _, possibleConstructor := range possibleConstructors {
		...
		if err != nil {
			// output to stderr, but don't treat as fatal error.
			fmt.Fprintf(os.Stderr, "genfuzzfuncs: failed to determine receiver type when looking for constructors: %v: %v", recv, err)
			continue
		}
...
func isExportedFunc(f *types.Func) bool {
	...
	if err != nil {
		...
		fmt.Fprintf(os.Stderr, "genfuzzfuncs: failed to determine exported: %v: %v", f, err)
		return false
	}
